...
public class Graph3DRenderer implements GLSurfaceView.Renderer {
    ...
    long milliseconds;
    boolean dirty, userRotate = false;
    ...
    public void setUpArray(){
    	...
    	float[] temp = new float[]{0,0,0,   0,0,2,
    			                   0,0,0,   0,2,0,
    			                   0,0,0,   2,0,0};
    	axisBuffer.put(temp);
    }
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    	...
    	p.add(x);
    	p.add(y);
    	AndyMath.setUpParser(p);
    	milliseconds = System.currentTimeMillis();
    	sp = context.getSharedPreferences("functions",0);
		functions = new ArrayList<String>();
		for(int i = 0; i < 6; i ++){
			String s = sp.getString("3d" + (i+1), "thisshouldntparse");
			if(AndyMath.isValid(s,new String[]{"x","y"})){
				functions.add(s);
			}
		}
		setUpArray();
